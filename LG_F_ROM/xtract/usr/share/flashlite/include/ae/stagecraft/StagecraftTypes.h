...
public:
	...
	virtual IFont *			GetIFontInterface() = 0;
...
{
public:
	virtual unsigned short Create(unsigned short fontSize, void** fontId) = 0;
	virtual unsigned short Destroy(void** fontId)						= 0;
	virtual unsigned short Select(void* fontId)							= 0;
	virtual void 			BeginTextDraw()								= 0;
	//.. GENE 20100531 void * pDestSurfaceÃß°¡ 	
	virtual unsigned short DeviceTextOut(void * pDestSurface, unsigned short* text, unsigned long textLen, unsigned short fontSize, short xPos, short yPos, 
							unsigned char red, unsigned char green, unsigned char blue, short xmin, short xmax, short ymin, short ymax) = 0;
	virtual bool 			ComplexLineBreak()							= 0;
	virtual void			EndTextDraw()								= 0;
	virtual bool			GetCharacterInfo()							= 0;
	virtual bool			GetBitmapInfo()								=0; 
	virtual void			GetFontMetrics(unsigned short fontSize, short* ascent, short* descent, unsigned short* singleByteSpaceWidth) = 0;
	virtual unsigned short 	GetTextWidth(unsigned short* text, unsigned long textLen, unsigned short fontSize, unsigned long * width) = 0;
	virtual void			change_fontpath() = 0;
...
// 20100813 DYKim
typedef int		(*PFNC_PLUGIN_EVALJS)				(void* pluginObject, const char* expression);
typedef int		(*PFNC_PLUGIN_DOFLASHCALL)		(void* pluginObject, const char* request);
typedef char*	(*PFNC_PLUGIN_GETDOMOBJECTID)	(void* pluginObject);
typedef int		(*PFNC_PLUGIN_STARTVIDEOPLAY)	(void* pluginObject);
typedef int		(*PFNC_PLUGIN_STOPVIDEOPLAY)		(void* pluginObject);
typedef int		(*PFNC_PLUGIN_GETURL)				(void* pluginObject, const char* url, const char *arg);
typedef int		(*PFNC_PLUGIN_NOTIFYERROR)		(void* pluginObject, short error);

struct PluginCallback
{
    PFNC_PLUGIN_EVALJS			pFncPlugin_EvalJS;
	PFNC_PLUGIN_DOFLASHCALL		pFncPlugin_DoFlashCall;
	PFNC_PLUGIN_GETDOMOBJECTID	pFncPlugin_GetDOMObjectId;

	PFNC_PLUGIN_STARTVIDEOPLAY 	pFncPlugin_StartVideoPlay;

	// getURL() function
	PFNC_PLUGIN_GETURL			pFncPlugin_GetURL;

	// OutOfMemory Notify
	PFNC_PLUGIN_NOTIFYERROR		pFncPlugin_NotifyError;

	PFNC_PLUGIN_STOPVIDEOPLAY 	pFncPlugin_StopVideoPlay;
};
...
struct PluginInstance
{
	...
	int					renderWidth;
	int					renderHeight;
	void*				mFullscreenWindow;
	int					mFullscreenWidth;
	int					mFullscreenHeight;
	void*				mParentWidget;
	int					mExposeId;
	void*				mScriptableObject;
	void*				userData;
	int/*bool*/				userDataUsed;
	int/*bool*/				destroyed;
	int					mInstanceNo;
	void*				mNetscapeFuncs;
	int/*bool*/				mVideoPlaying;
	int					bPriorityModified;
	int					mInWindow;
	int					mIsFullBrowser;
	int					bWindowSizeAccumulated;
	int					bFirstFrameDrawn;
};
