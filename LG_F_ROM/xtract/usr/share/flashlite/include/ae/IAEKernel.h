...
public:
	inline void GetPos(int &pos_x, int &pos_y) 	{ pos_x = m_pos_x ; pos_y = m_pos_y; }
	inline void SetPos(int pos_x, int pos_y)		{ m_pos_x = pos_x; m_pos_y = pos_y;	 }
...
public:
	...
	virtual void WriteConsoleOutputStringAddon(const char * pFormatString, ...) = 0;
...
// AEKernel.h platform independent #defines
	...
	#ifdef SC_DTV
    #define AEPRINT IAEKernel::GetKernel()->WriteConsoleOutputStringAddon 
	#else 
	#define AEPRINT IAEKernel::GetKernel()->WriteConsoleOutputString
	#endif
