...
#define MAX_TEXT 32
#define LIRC_MTK_MSG_KEY   ((key_t)4851)
#define DFB_KEY_MSG_TYPE   (2)

struct lirc_mtk_msg_st {
    long int lirc_mtk_msg_type;
    char bt_name[MAX_TEXT];
    unsigned long bt_code;
    int rep;
};

static int key_msg_id;
...
static inline void
mouse_postion( SerialMouseData *data, int dx, int dy )
{
     data->x_motion.axisabs = dx;
     data->y_motion.axisabs = dy;
}
...
static inline void
mouse_position_realize( SerialMouseData *data )
{
     if (data->x_motion.axisabs) {
          data->x_motion.flags = DIEF_AXISABS;
          dfb_input_dispatch( data->device, &data->x_motion );
          data->x_motion.axisabs = 0;
     }

     if (data->y_motion.axisabs) {
          data->y_motion.flags = DIEF_AXISABS;
          dfb_input_dispatch( data->device, &data->y_motion );
          data->y_motion.axisabs = 0;
     }
}
...
mouseEventThread_ms( DirectThread *thread, void *driver_data )
{
    ...
    DFBInputEvent    evt;
    DFBInputDeviceButtonIdentifier last_button = DIBI_LAST;
    DFBInputDeviceKeySymbol last_key = DIKS_NULL;
    ...
    while ((readlen = read( data->fd, buf, 256 )) >= 0 || errno == EINTR) {
        ...
        switch ((unsigned int)((buf[4] << 8) | buf[5]))
        {
#if 0
            case 0x1203: /* Touch pad: position */
            {
                switch(buf[10])
                {
                    case 0x12: /* left area */
                        dx = (int)((buf[12] << 8) | buf[13]);
                        dy = (int)((buf[14] << 8) | buf[15]);
                        mouse_postion( data, dx, dy );
                        mouse_position_realize( data );
                        break;
                    case 0x22: /* right area */
                        dx = (int)((buf[16] << 8) | buf[17]);
                        dy = (int)((buf[18] << 8) | buf[19]);
                        mouse_postion( data, dx, dy );
                        mouse_position_realize( data );
                        break;
                    default:
...
#else
            case 0x1203: /* Touch pad: direction */
            {
                if (0x02 == buf[10])
                {
                    if (last_button == DIBI_MIDDLE)
                    {
                        evt.type       = DIET_KEYRELEASE;
                        evt.key_symbol = last_key;
                        last_button    = DIBI_LAST;
                    }
                    else
                    {
                        continue;
                ...
                else if(0x12 == buf[10] || 0x22 == buf[10] || 0x32 == buf[10])
                {
                    switch(buf[11])
                    {
                        case 0x01: /* Touch pad: direction: right */
                        case 0x10:
                            evt.key_symbol = DIKS_CURSOR_RIGHT;
                            break;
                        case 0x03: /* Touch pad: direction: up */
                        case 0x30:
                            evt.key_symbol = DIKS_CURSOR_UP;
                            break;
                        case 0x05: /* Touch pad: direction: left */
                        case 0x50:
                            evt.key_symbol = DIKS_CURSOR_LEFT;
                            break;
                        case 0x07: /* Touch pad: direction: down */
                        case 0x70:
                            evt.key_symbol = DIKS_CURSOR_DOWN;
                            break;
#if 0
                        case 0x02: /* Touch pad: direction: up right */
                        case 0x20:
                            evt.key_symbol = DIKS_CURSOR_UP_RIGHT;
                            break;
                        case 0x04: /* Touch pad: direction: left up */
                        case 0x40:
                            evt.key_symbol = DIKS_CURSOR_LEFT_UP;
                            break;
                        case 0x06: /* Touch pad: direction: left down */
                        case 0x60:
                            evt.key_symbol = DIKS_CURSOR_LEFT_DOWN;
                            break;
                        case 0x08: /* Touch pad: direction: down right */
                        case 0x80:
                            evt.key_symbol = DIKS_CURSOR_DOWN_RIGHT;
                            break;
#endif
                        default:
                            continue;
                    ...
                    if (last_button == DIBI_MIDDLE && evt.key_symbol == last_key)
                    {
                        evt.type       = DIET_KEYRELEASE;
                        last_button    = DIBI_LAST;
                    }
                    else
                    {
                        ...
                        last_button    = DIBI_MIDDLE;
                    }

                    last_key       = evt.key_symbol;
                }
                else
                {
                    continue;
                ...
                strcpy(msgt.bt_name, "PanasonicIR");
                msgt.lirc_mtk_msg_type = DFB_KEY_MSG_TYPE;
                msgt.bt_code = (unsigned long)evt.key_symbol;
                msgt.rep = evt.type;

                printf("      <<< dfb msgt.bt_code=0x%x msgt.rep=%d >>>\n", msgt.bt_code, msgt.rep);

                if(msgsnd(key_msg_id, (void *)&msgt, (sizeof(struct lirc_mtk_msg_st) - sizeof(long int)), IPC_NOWAIT) == -1)
                {
                   D_PERROR("DirectFB/SerialMouse: msgsnd failed, errno=%d!\n", errno);
                }
            ...
            case 0x0a03: /* Mouse button & Touch pad: click & Hot key*/
            {
                switch ((unsigned int)((buf[6] << 24) | (buf[7] << 16) | (buf[8] << 8) | buf[12]))
                {
                    case 0x00000009: /* both two use it: release */
                        if ((last_button == DIBI_LEFT) || (last_button == DIBI_RIGHT))
                        {
                            evt.type    = DIET_BUTTONRELEASE;
                            evt.button  = (last_button == DIBI_LEFT) ? DIBI_LEFT : DIBI_RIGHT;
                            evt.flags   = DIEF_NONE;
                            last_button = DIBI_LAST;
                        }
                        else if (last_button == DIBI_MIDDLE)
                        {
                            evt.type       = DIET_KEYRELEASE;
                            evt.flags      = DIEF_KEYSYMBOL;
                            evt.key_symbol = last_key;
                        ...
                        break;
                    case 0x0000040d: /* Mouse: left */
                        evt.type    = DIET_BUTTONPRESS;
                        evt.button  = DIBI_LEFT;
                        evt.flags   = DIEF_NONE;
                        last_button = DIBI_LEFT;
                        break;
                    case 0x00000801: /* Mouse: right */
                        evt.type    = DIET_BUTTONPRESS;
                        evt.button  = DIBI_RIGHT;
                        evt.flags   = DIEF_NONE;
                        last_button = DIBI_RIGHT;
                        break;
                    case 0x00000108: /* Touch pad: click: left */
                    case 0x0000020b: /* Touch pad: click: right */
                        evt.type       = DIET_KEYPRESS;
                        evt.flags      = DIEF_KEYSYMBOL;
                        evt.key_symbol = DIKS_SELECT;
                        last_button    = DIBI_MIDDLE;
                        last_key       = DIKS_SELECT;
                        break;
                    case 0x0002000b: /* Hot key: menu */
                        evt.type       = DIET_KEYPRESS;
                        evt.flags      = DIEF_KEYSYMBOL;
                        evt.key_symbol = DIKS_INFO;
                        last_button    = DIBI_MIDDLE;
                        last_key       = DIKS_INFO;
                        break;
                    case 0x80000089: /* Hot key: esc */
                        evt.type       = DIET_KEYPRESS;
                        evt.flags      = DIEF_KEYSYMBOL;
                        evt.key_symbol = DIKS_RETURN;
                        last_button    = DIBI_MIDDLE;
                        last_key       = DIKS_RETURN;
                        break;
                    default:
                        continue;
                }
                ...
                strcpy(msgt.bt_name, "PanasonicIR");
                msgt.lirc_mtk_msg_type = DFB_KEY_MSG_TYPE;
                msgt.bt_code = (unsigned long)(evt.type == DIET_BUTTONPRESS | evt.type == DIET_BUTTONRELEASE ? evt.button : evt.key_symbol);
                msgt.rep = evt.type;
            ...
            case 0x0e02: /* Jog ball */
            {
                dx = (short)((buf[12] << 8) | buf[13]);
                dy = (short)((buf[14] << 8) | buf[15]);
...
driver_get_info( InputDriverInfo *info )
{
     ...
     snprintf( info->name,
               DFB_INPUT_DRIVER_INFO_NAME_LENGTH, "Panasonic Remote Controller" );
                    ...
                    void            **driver_data )
{
     ...
     /* open device */
     flags = O_RDONLY;
     fd = open( ITF_DEV, flags );
     if (fd < 0) {
          D_PERROR( "DirectFB/SerialMouse: Error opening '%s'!\n", dfb_config->mouse_source );
          return DFB_INIT;
     }

     key_msg_id = msgget(LIRC_MTK_MSG_KEY, (0666|IPC_CREAT));
     if(key_msg_id == -1)
     {
     	D_PERROR("DirectFB/SerialMouse: msgget failed with error : %d \n", errno);
     	return DFB_FAILURE;
     }
     ...
     snprintf( info->desc.name, DFB_INPUT_DEVICE_DESC_NAME_LENGTH, "Panasonic Remote Controller" );

     snprintf( info->desc.vendor, DFB_INPUT_DEVICE_DESC_VENDOR_LENGTH, "MediaTek" );

     info->prefered_id     = DIDID_KEYBOARD;
     info->desc.type       = DIDTF_MOUSE | DIDID_KEYBOARD;
     info->desc.caps       = DICAPS_AXES | DICAPS_BUTTONS | DICAPS_KEYS;
     info->desc.max_axis   = DIAI_Y;
     info->desc.max_button = DIBI_RIGHT;

     /* start input thread */
     data->thread = direct_thread_create( DTT_INPUT, mouseEventThread_ms,
                                          data, "Panasonic RC Input" );
