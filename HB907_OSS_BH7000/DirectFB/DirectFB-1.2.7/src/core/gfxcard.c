                              ...
                              CardState *state )
{
     ...
     /* Yan: for Netflix: a margin in the left not flicker when start */
     if(drect->x == -48 && drect->y == -27 && drect->w == 961 && drect->h == 540)
     {
        drect->x = 0;
        drect->y = 0;
        drect->w = 960;
                        ...
                        CoreFont *font, CardState *state )
{
     ...
     int kern_y;
     int blit = 0;
     int hw_clipping = D_FLAGS_IS_SET( card->caps.flags, CCF_CLIPPING ) ||
                       D_FLAGS_IS_SET( card->caps.clip, DFXL_BLIT );
