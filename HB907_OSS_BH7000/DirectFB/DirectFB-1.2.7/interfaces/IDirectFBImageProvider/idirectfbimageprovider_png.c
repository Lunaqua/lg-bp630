...
#if PNG_LIBPNG_SUPPORT_HWDECODE
...
#if PNG_LIBPNG_SUPPORT_HWDECODE 
static IDirectFBImageProvider_PNG_data *PngSrcData;
static char b_use_hw = 1;

static void
png_hw_info_callback (png_structp png_read_ptr,
...
#if PNG_LIBPNG_SUPPORT_HWDECODE
static void 
png_readdata_callback(png_structp png_ptr, png_bytep to_data, png_size_t length)
{
    png_size_t check;

    if( PngSrcData->buffer->HasData( PngSrcData->buffer ) == DFB_OK )
    {
        PngSrcData->buffer->GetData( PngSrcData->buffer, length, to_data, &check );

        if (check != length)
        {
             /* set end stage */
             PngSrcData->stage = STAGE_END;
             png_ptr->fgEndOfFile = 1;
			 PNG_LOG("%s DFB- STAGE_END\n", __FUNCTION__);
        }
    }
}
#endif
           ...
           ... )
{
...
#if PNG_LIBPNG_SUPPORT_HWDECODE
     b_use_hw = 1;
     //data->end_info_ptr = png_create_info_struct( data->png_ptr );

     PngSrcData = data;

     png_set_read_fn(data->png_ptr, NULL, png_readdata_callback);

     data->png_ptr->fgUseHWdecode = HWEN;
     data->png_ptr->fgWaitHWresource = 0x1;

     png_read_info(data->png_ptr, data->info_ptr);
     png_hw_info_callback(data->png_ptr, data->info_ptr, data);

#else /* PNG_LIBPNG_SUPPORT_HWDECODE */
     ...
     if (ret)
          goto error;
#endif /* PNG_LIBPNG_SUPPORT_HWDECODE */
...
#if PNG_LIBPNG_SUPPORT_HWDECODE
     if (data->png_ptr->fgUseHWdecode)//b_use_hw)
         png_destroy_read_struct( &data->png_ptr, &data->info_ptr, &data->end_info_ptr );
     else
#else
         png_destroy_read_struct( &data->png_ptr, &data->info_ptr, NULL );
#endif
                                     ...
                                     const DFBRectangle     *dest_rect )
{
     ...
     if (!dst_surface)
     {
         PNG_LOG("DFB-PNG-DFB_FAILURE %s--%d\n", __FUNCTION__, __LINE__);
     ...
     if (setjmp(data->png_ptr->jmpbuf )) 
	 {
		  ...
		  data->buffer->GetLength( data->buffer, &buf_len );
		  PNG_LOG("TEST********** %s--%d--[buf_len:%d]\n", __FUNCTION__, __LINE__, buf_len);
          data->buffer->SeekTo( data->buffer, buf_len);
          PNG_LOG("TEST********** Seek to End\n");
	 ...
	 {
	      PNG_LOG("%s Call push_data_until_stage --%d\n", __FUNCTION__, __LINE__);   			     
...
#if PNG_LIBPNG_SUPPORT_HWDECODE
          if ( lock.phys > 0xC0000000 )
          {
              printf("------DirectFB decoding with png HW: %d x %d------\n", data->width, data->height);

              b_use_hw = 1;
              data->png_ptr->u4FgTestMode = 0;
              data->png_ptr->u4MemoryMode = 0;
              data->png_ptr->width = data->width;
              data->png_ptr->height = data->height;
              data->png_ptr->u4DstHeight = dst_surface->config.size.h;
              data->png_ptr->u4DstWidth = dst_surface->config.size.w;
              data->png_ptr->u4DstPitch = lock.pitch;
              png_read_image2(data->png_ptr, lock.phys);
			  if ((data->stage != STAGE_ERROR) &&(data->stage != STAGE_END))
              {
                 data->buffer->GetLength( data->buffer, &buf_len );
       		     PNG_LOG("TEST**********%s--%d--[buf_len:%d]\n", __FUNCTION__, __LINE__, buf_len);
                 data->buffer->SeekTo( data->buffer, buf_len);
              ...
              png_read_end(data->png_ptr, data->end_info_ptr); 
              if(data->stage != STAGE_END)
              {
                    PNG_LOG("------DirectFB force set  STAGE_END\n");
                    data->stage = STAGE_END;
              }
          }
          else
          {
              printf("------DirectFB decoding with png SW: %d x %d------\n", data->width, data->height);
              b_use_hw = 0;
              png_destroy_read_struct( &data->png_ptr, &data->info_ptr, data->end_info_ptr );                  
              ...
              if (!data->png_ptr)
              {
                   PNG_LOG("DFB-png_create_read_struct failed\n");
