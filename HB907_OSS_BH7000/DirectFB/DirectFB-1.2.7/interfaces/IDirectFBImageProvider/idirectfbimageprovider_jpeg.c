...
#if JDEC_LIBJPG_SUPPORT_HWDECODE
     if(nbytes < JPEG_PROG_BUF_SIZE)
     {
          cinfo->fgReadEnd = TRUE;
     }
#endif 
...
#if JDEC_LIBJPG_SUPPORT_HWDECODE
     cinfo.tHwInstInf.fgUseHWdecode = 0;
#endif
                                      ...
                                      const DFBRectangle     *dest_rect )
{
     ...
     if (!dst_surface)
     {
          JPG_LOG("DFB-JPG-DFB_FAILURE %s--%d\n", __FUNCTION__, __LINE__);
     ...
     if (!data->image) {
          ...
          {
              printf("------DirectFB decoding with jpeg HW: %d x %d------\n", data->width, data->height);

              cinfo.tHwInstInf.fgUseHWdecode = HWEN;
              JPG_LOG("call by DFB hw\n");
              jpeg_create_decompress(&cinfo);
              jpeg_buffer_src(&cinfo, data->buffer, 0);
              jpeg_read_header(&cinfo, TRUE);

              cinfo.out_color_space = JCS_ARGB;
              cinfo.output_components = 3;
              cinfo.output_width = dst_surface->config.size.w;
              cinfo.output_height = dst_surface->config.size.h;
              ...
              jpeg_start_read_image(&cinfo, lock.phys, lock.pitch, JCS_ARGB);
#if 1
			  //if ((data->stage != STAGE_ERROR) &&(data->stage != STAGE_END))
              {
                 data->buffer->GetLength( data->buffer, &buf_len );
          ...
          {
#if  JDEC_LIBJPG_SUPPORT_HWDECODE
              printf("------DirectFB decoding with jpeg SW: %d x %d------\n", data->width, data->height);
              cinfo.tHwInstInf.fgUseHWdecode = 0;
#endif
              jpeg_create_decompress(&cinfo);
              jpeg_buffer_src(&cinfo, data->buffer, 0);
