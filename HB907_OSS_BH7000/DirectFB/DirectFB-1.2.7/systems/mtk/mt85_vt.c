...
*
* \par Description
*   The function bodies of mt85 vt
*
*/
...
D_DEBUG_DOMAIN( VT, "MT85/VT", "MT85 System Module VT Handling" );
     ...
     while (true) {
          ...
          switch (dfb_vt->vt_sig) {
               ...
               case SIG_SWITCH_FROM:
                    if (dfb_core_suspend( dfb_mt85->core ) == DFB_OK) {
               ...
               case SIG_SWITCH_TO:
                    if (dfb_core_resume( dfb_mt85->core ) == DFB_OK) {
                         if (ioctl( dfb_vt->fd, VT_RELDISP, VT_ACKACQ ) < 0)
                              D_PERROR( "DirectFB/fbdev/vt: VT_RELDISP failed\n" );
...
vt_get_fb( int vt )
{
     ...
     if (ioctl( dfb_mt85->fd, FBIOGET_CON2FBMAP, &c2m )) {
          D_PERROR( "DirectFB/MT85/vt: "
...
vt_set_fb( int vt, int fb )
{
     ...
     if (fstat( dfb_mt85->fd, &sbf )) {
          D_PERROR( "DirectFB/MT85/vt: Could not fstat fb device!\n" );
          return;
     }
     ...
     if (ioctl( dfb_mt85->fd, FBIOPUT_CON2FBMAP, &c2m ) < 0) {
          D_PERROR( "DirectFB/MT85/vt: "
