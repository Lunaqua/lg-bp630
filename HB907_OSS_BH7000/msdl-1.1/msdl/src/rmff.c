...
struct rmff_fileheader_t *new_rmff_fileheader(uint32_t num_headers)
{
    ...
    struct rmff_fileheader_t *fileheader = xmalloc(sizeof(struct rmff_fileheader_t));
				   ...
				   uint16_t flags )
{
    ...
    struct rmff_prop_t *prop = xmalloc(sizeof(struct rmff_prop_t));
				  ...
				  const uint8_t *type_specific_data )
{
    ...
    struct rmff_mdpr_t *mdpr = xmalloc(sizeof(struct rmff_mdpr_t));
    ...
    mdpr->type_specific_data = xmalloc(sizeof(char)*type_specific_len);
...
struct rmff_cont_t *new_rmff_cont(const char *title, const char *author, const char *copyright, const char *comment)
{
    ...
    struct rmff_cont_t *cont = xmalloc(sizeof(struct rmff_cont_t));
					...
					uint32_t next_data_header)
{
    ...
    struct rmff_data_t *data = xmalloc(sizeof(struct rmff_data_t));
...
void rmff_print_header(struct rmff_header_t *h)
{
	...
	for(i = 0 ; (stream = h->streams[i]) ; i++) {
	    ...
	    for(j = 0 ; j < stream->type_specific_len ; j++) {
		display(MSDL_DBG,"%02x",stream->type_specific_data[j]);
	    }
	    display(MSDL_DBG,"\n");
	}
    }
...
void rmff_fix_header(struct rmff_header_t *h)
{
	...
	h->data = xmalloc(sizeof(struct rmff_data_t));
	...
	h->fileheader = xmalloc(sizeof(struct rmff_fileheader_t));
	...
	display(MSDL_DBG | MSDL_ERR,"rmff_fix_header: setting num_headers from %i to %i\n",
