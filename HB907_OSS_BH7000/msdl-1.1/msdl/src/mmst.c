...
static struct mmst_ctrl_t *new_mmst_ctrl_t(void)
{
    struct mmst_ctrl_t *mctrl = xmalloc(sizeof(struct mmst_ctrl_t));
    mctrl->seq_num = 0;
 ...
 * send command
 */
static void send_command(struct stream_t *stream,int command,uint32_t switches,
			 uint32_t extra,int length,uint8_t *data)
{
    int len8;
    uint8_t *cmd;
    int i;
    ...
    cmd = (uint8_t *)xmalloc(len8 * 8 + 0x30); // 0x30 for header.
  
    /* put data in little endian */
    put32_le(cmd + 0x00,0x00000001); /* start sequence */
    put32_le(cmd + 0x04,0xB00BFACE); /* BOOBFACE */
    put32_le(cmd + 0x08,len8*8 + 32); 
    put32_le(cmd + 0x0C,0x20534d4d); /* "MMS " */
    put32_le(cmd + 0x10,len8 + 4); 
    put32_le(cmd + 0x14,stream->stream_ctrl->mmst_ctrl->seq_num);
    stream->stream_ctrl->mmst_ctrl->seq_num++;
    put32_le(cmd + 0x18,0x00); /* unknown */
    put32_le(cmd + 0x1C,0x00); /* unknown */
    put32_le(cmd + 0x20,len8+2);
    put32_le(cmd + 0x24,0x00030000 | command); /* dir | command */
    put32_le(cmd + 0x28,switches);
    put32_le(cmd + 0x2C,extra);
    ...
    for( i = 0; i < len8 * 8 + 0x30; i ++) {
	display(MSDL_DBG,"%02x",cmd[i]);
    }
    display(MSDL_DBG,"\n=------------------------------------------------=\n");
  
  
    if(xsend(stream->sock,cmd,len8*8+0x30) != (len8*8+0x30)){
	perror("send() failed");
    }
...
static int mmst_get_asf_header(struct stream_t *stream, uint8_t *header)
{
	...
	if(pre_header[4] == 0x02) { /*   header packet   */
	    ...
	    if(command == 0x1b) {
		send_command(stream, 0x1b, 0,0,0, combuf);
	    }
	    free(combuf);
	}
    }
...
int mmst_streaming_start(struct stream_t *stream)
{
    ...
    unescape_path = (char *)xmalloc(strlen(path) + 1);
  
  
    /* remove something like '%41' in url. */
    url_unescape_string(unescape_path,path);
    path = unescape_path;
     ...
     */
    snprintf((char *)buffer2,BUFSIZE_1K - 1,"\034\003NSPlayer/7.0.0.1956; {33715801-BAB3-9D85-24E9-03B90328270A}; Host: %s",url->hostname);
  
    string_utf16((char *)buffer,(char *)buffer2,
		 strlen((char *)buffer2));
  
    /*  send !! */
    send_command(stream,1,0,0x0004000b,strlen((char *)buffer2)*2+2,
		 buffer);
    ...
    memset(buffer,0,8);
    send_command(stream,2,0,0,24*2+10,buffer);
    ...
    memset(buffer,0,8);
    send_command(stream,5,0,0,strlen(path) * 2 + 10, buffer);
    ...
    memset(buffer,0,40);
    buffer[0x20] = 2;
    send_command(stream,0x15,1,0,40,buffer);
    ...
    for(i = 0;i < mmst_ctrl->hinfo->streams->n_audio; i++) { /* if more than 1. */
	buffer[pos + 2] = 0xFF;
	buffer[pos + 3] = 0xFF;
	buffer[pos + 4] = mmst_ctrl->hinfo->streams->audio_streams[i];
	buffer[pos + 5] = mmst_ctrl->hinfo->streams->audio_streams[i] >> 8;
    
	if(mmst_ctrl->hinfo->streams->audio_streams[i] ==
	   mmst_ctrl->hinfo->streams->audio_id) {
	    display(MSDL_VER,"audio stream [0x%02x] enabled\n",
		    mmst_ctrl->hinfo->streams->audio_streams[i]);
	    buffer[pos + 6] = 0x00; /* enable stream  */
	    buffer[pos + 7] = 0x00;
	}
	else {
	    display(MSDL_VER,"audio stream [0x%02x] disabled\n",
		    mmst_ctrl->hinfo->streams->audio_streams[i]);
	    buffer[pos + 6] = 0x02; /* disable stream */
	    buffer[pos + 7] = 0x00;
	}
	pos += 6;
    }
    /* enable/disable video streams */
    for(i = 0;i < mmst_ctrl->hinfo->streams->n_video; i++) { /* if more than 1. */
	buffer[pos + 2] = 0xFF;
	buffer[pos + 3] = 0xFF;
	buffer[pos + 4] = mmst_ctrl->hinfo->streams->video_streams[i];
	buffer[pos + 5] = mmst_ctrl->hinfo->streams->video_streams[i] >> 8;
    
	if(mmst_ctrl->hinfo->streams->video_streams[i] ==
	   mmst_ctrl->hinfo->streams->video_id) {
	    display(MSDL_VER,"video stream [0x%02x] enabled\n",
		    mmst_ctrl->hinfo->streams->video_streams[i]);
	    buffer[pos + 6] = 0x00; /* enable stream  */
	    buffer[pos + 7] = 0x00;
	}
	else {
	    display(MSDL_VER,"video stream [0x%02x] disabled\n",
		    mmst_ctrl->hinfo->streams->video_streams[i]);
	    buffer[pos + 6] = 0x02; /* disable stream */
	    buffer[pos + 7] = 0x00;
	}
    ...
    send_command(stream,0x33,mmst_ctrl->num_stream_ids,
		 /*0xFFFF | ((mmst_ctrl->hinfo->streams->audio_id) ?
		   mmst_ctrl->hinfo->streams->audio_id :
		   mmst_ctrl->hinfo->streams->video_id) << 16,*/
		 0,
		 pos + 2,
		 buffer);
    //  get_answer(stream);
...
static int mmst_get_media_packet(struct stream_t *stream, uint8_t *buffer, size_t max_size)
{
	...
	if(pre_header[4] == 0x04) { /*   media packet   */
		...
		display(MSDL_VER | MSDL_ERR,"unknown command %x (ignore this)\n",command);
	    }
...
struct stream_t *mmst_streaming_init(struct url_t *url)
{
