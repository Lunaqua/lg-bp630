...
static struct list_h *set_options(struct options_t *options,int argc,char **argv)
{
    ...
    */
	...
	ch = getopt_long(argc,argv,"o:p:b:s:r:m:n:hVvq",long_options,&option_index);
	...
	case 'o': /* output file name specification */
	    options->local_filename = strdup(optarg);
	    break;
    ...
    /*
      show warnings
    */
    if(options->local_filename && target_count > 1) {
	display(MSDL_ERR,
		"warning: output filename specified but trying to\n"
		"         download more than 1 files. name which\n"
		"         you specfified will be applied to only\n"
