...
char *rtsp_request_options(struct rtsp_header_t *rtsp_hdr,struct url_t *,
			   char *what);
			...
			uint8_t *buffer, size_t buffer_size)
{
	...
	if(stream_ctrl->rtsp_ctrl->get_media_packet != NULL) {
...
void rtsp_set_range_field(struct rtsp_header_t *rtsp_hdr,const char *rangestr)
{
	...
	if(rtsp_npt_range_valid(rangestr,&reason)) { /* valid */
		...
		display(MSDL_ERR,
			"option \"-r %s\" is invalid as rtsp range request\n"
			": %s\n"
			"send \"%s\" instead\n",
...
void rtsp_set_speed_field(struct rtsp_header_t *rtsp_hdr,const char *speedstr)
{
	...
	if(rtsp_speed_valid_and_guess(speedstr,&guessed_speed,&reason)) {
	    ...
	    display(MSDL_ERR,
		    "option \"-s %s\" is invalid as rtsp speed request\n"
		    ": %s\n"
		    "send \"%s\" instead\n",
...
char *rtsp_time_to_npt_time(const char *str,char **reason_ret)
{
	...
	if('0' <= str[i] && str[i] <= '9') {
	...
	else if(str[i] == ':') {     /* colon can be a separator */
		...
		tolower(str[i]) == 'y'|| tolower(str[i]) == 'd') {
	...
	uint64_t sec = 0;
	npt_time = xmalloc(BUFSIZE_1K + strlen(str_after_dot));
	/* hhmmss format */
	/*
	  sec   = sep_time[SEC] % 60;
    ...
    */
	...
	if(p == NULL) { /* really inacceptable. */
		...
		*reason_ret = reason;
	    ...
	    npt_range_str = xmalloc(strlen(npt_time_str) + 2);
	    npt_range_str[0] = '-';
	    ...
	    original_start_time_str = xmalloc(start_str_len + 1);
	    ...
	    npt_range_str = xmalloc(strlen(npt_start_time_str) +
...
int rtsp_npt_range_valid(const char *str,char **reason_ret)
{
	...
	/* check start string */
	npttime_str = xmalloc(start_str_len + 1);
...
/*
 * return true if Speed: string, according to rfc2326
 * return value ... 1: valid   0: not valid
 *   reason_ret ... "": no error   some string: error reason
 */
int rtsp_speed_valid_and_guess(const char *str,int *guessed_speed,char **reason_ret)
{
    /* 1*DIGIT [ "." *DIGIT ] */
    int i = 0;
    int len = strlen(str);
...
int rtsp_recv_header(struct stream_t *stream, struct rtsp_header_t *rtsp_hdr)
{
	...
	if(i <= 0) {
	    display(MSDL_ERR,"recv error: recv_data() returned %d\n",i);
	    goto failed;
	}
	total += i;
    ...
    ret = rtsp_parse_response(stream_ctrl->rtsp_ctrl,rtsp_hdr);
		      ...
		      struct rtsp_header_t *rtsp_hdr)
{
    ...
    ret = xsend(stream->sock,rtsp_hdr->buffer,rtsp_hdr->buffer_len);
...
int rtsp_connect(struct stream_t *stream)
{
    ...
    if(url->port == 0) {
	url->port = RTSP_PORT; /* RTSP default port */
    }
    ...
    /* make mrl */
    rtsp_make_mrl(rtsp_ctrl,url);

    sock = server_connect(url->hostname,url->port);
    if(sock < 0) { /* couldn't connect for some reason. (port closed maybe) */
	goto failed;
    }
    
    stream->sock = sock; /* save socket id */
    return sock;
 ...
 */
int rtsp_make_mrl(struct rtsp_ctrl_t *rtsp_ctrl,struct url_t *url)
{
    int len;
    len = strlen(url->hostname) + strlen(url->filepath) + 32; /* 32 is enough */
  
    rtsp_ctrl->mrl = (char *)xmalloc(len);
    snprintf(rtsp_ctrl->mrl,len,"rtsp://%s:%i%s",
	     url->hostname,url->port,url->filepath);
    return 1;
}
			...
			struct rtsp_header_t *rtsp_hdr)
{
    ...
    if(rtsp_response_parse(rtsp_hdr) < 0) {
	display(MSDL_ERR,"RTSP header parse error\n");
	return -1;
    }
	...
	if(rtsp_ctrl->cseq - 1 != answer_cseq) {
	    /* -1 because cseq is incremented in build_request */
	    display(MSDL_DBG | MSDL_ERR,"warning: Cseq mismatch, got %u, expected %u\n",
	...
	if(rtsp_ctrl->server) { /* already has it */
		...
		display(MSDL_VER | MSDL_ERR,"warning: got different server\n"
			"    old: %s\n"
	...
	if(rtsp_ctrl->session) {
		...
		display(MSDL_VER | MSDL_ERR,"warning: got different session\n"
			"    old: [%s]\n"
	...
	if(rtsp_ctrl->challenge) {
		...
		display(MSDL_ERR,"warning: got different client challenge\n"
			"    old: %s\n"
...
char *rtsp_get_next_field(struct rtsp_header_t *rtsp_hdr)
{
	...
	if(!strncasecmp(field->field_name,rtsp_hdr->field_search,
...
void rtsp_set_field(struct rtsp_header_t *rtsp_hdr, const char *field_name)
{
    ...
    new_field = xmalloc(sizeof(struct rtsp_field_t));
...
void rtsp_set_uri(struct rtsp_header_t *rtsp_hdr,const char *uri)
{
    ...
    rtsp_hdr->uri = xmalloc(strlen(uri) + 1);
...
void rtsp_set_method(struct rtsp_header_t *rtsp_hdr, const char *method)
{
    ...
    rtsp_hdr->method = xmalloc(strlen(method) + 1);
...
int rtsp_200ok(struct stream_t *stream,int cseq,char *session)
{
    ...
    ret = xsend(stream->sock,buf,strlen(buf));
...
/* OPTIONS */
char *rtsp_request_options(struct rtsp_header_t *rtsp_hdr,struct url_t *url,
			   char *what)
{
    rtsp_set_method(rtsp_hdr,"OPTIONS");
  
    if(what) { /* use what as uri */
	rtsp_set_uri(rtsp_hdr,what);
    }

    else {
	rtsp_hdr->uri = (char *)xmalloc(strlen(url->hostname) + 16);
	sprintf(rtsp_hdr->uri,"rtsp://%s:%i",url->hostname,url->port);
    }
...
/* DESCRIBE */
char *rtsp_request_describe(struct rtsp_header_t *rtsp_hdr,struct url_t *url,
			    char *what)
{
	...
	rtsp_hdr->uri = (char *)xmalloc(strlen(url->hostname) + 
					strlen(url->filepath) + 16);
	sprintf(rtsp_hdr->uri,"rtsp://%s:%i%s",
		url->hostname,url->port,url->filepath);
    }
    return (rtsp_build_request(rtsp_hdr));
}
...
/* SETUP */
char *rtsp_request_setup(struct rtsp_header_t *rtsp_hdr,struct url_t *url,
			 char *what)
{
...
/* SET_PARAMETER */
char *rtsp_request_set_parameter(struct rtsp_header_t *rtsp_hdr,
				 struct url_t *url,
				 char *what)
{
...
/* GET_PARAMETER */
char *rtsp_request_get_parameter(struct rtsp_header_t *rtsp_hdr,
				 struct url_t *url,
				 char *what)
{
...
/* PLAY */
char *rtsp_request_play(struct rtsp_header_t *rtsp_hdr,
			struct url_t *url,
			char *what)
{
...
/* TEARDOWN */
char *rtsp_request_teardown(struct rtsp_header_t *rtsp_hdr,struct url_t *url,
			    char *what)
{
...
char *rtsp_build_request(struct rtsp_header_t *rtsp_hdr)
{
    ...
    rtsp_hdr->buffer = xmalloc(len + 1); // 1 for '\0'
...
void rtsp_streaming_close(struct stream_t *stream)
{
	...
	rtsp_request_teardown(rtsp_hdr,stream->url,NULL);
	ret = rtsp_send_request_and_free(stream,rtsp_hdr);
    }
  
    if(stream->sock > 0) { /* valid socket --> close */
	close(stream->sock);
    }
...
struct stream_t *rtsp_streaming_init(struct url_t *url)
{
