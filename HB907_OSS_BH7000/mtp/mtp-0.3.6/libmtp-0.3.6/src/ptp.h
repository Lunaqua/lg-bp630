...
#define PTP_RC_InvalidSemHandle		0x2024
...
//added by bin liu
uint16_t ptp_create_transaction_sem(char *sem_name,sem_t **ppSem);

uint16_t ptp_del_transaction_sem(sem_t *pSem);

uint16_t ptp_wait_transaction_sem(sem_t *pSem);

uint16_t ptp_release_transaction_sem(sem_t *pSem);


#define MTP_LOG_ON 1
#if MTP_LOG_ON
extern int i_enable_mtp_log;
#define MTP_Printf(...)  if((i_enable_mtp_log==1)||(i_enable_mtp_log==3)) {printf("LIBMTP :"); printf(__VA_ARGS__);}
#define BULK_Printf(...) if((i_enable_mtp_log==2)||(i_enable_mtp_log==3)) {printf("BULK :"); printf(__VA_ARGS__);}
#else
#define MTP_Printf(...)
#define BULK_Printf(...)
#endif
