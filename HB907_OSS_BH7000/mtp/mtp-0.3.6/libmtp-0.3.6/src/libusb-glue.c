...
#define CFG_MASS_STORAGE_FIRST  1 
#define MTP_DEVICE_CHK_ENHANCE 1
...
struct mtpdevice_attched_struct
{
    PTPParams *params; 
    struct mtpdevice_attched_struct *pt_next;  
};
typedef struct mtpdevice_attched_struct mtpdevice_attched_list_t;
static mtpdevice_attched_list_t *_pt_mtp_attched_device = NULL;

int ptp_save_dev_to_list(PTPParams *pt_ptp_parm)
{  
    mtpdevice_attched_list_t *pt_cur_node   = NULL;
    mtpdevice_attched_list_t *pt_pre_node   = NULL;
    
    MTP_Printf("Enter %s \n",__FUNCTION__);

    if(_pt_mtp_attched_device == NULL)
    {
      _pt_mtp_attched_device =(mtpdevice_attched_list_t *) malloc(sizeof(mtpdevice_attched_list_t));
      if(_pt_mtp_attched_device!= NULL)
      {
       memset(_pt_mtp_attched_device,0,sizeof(mtpdevice_attched_list_t));
       _pt_mtp_attched_device->params = pt_ptp_parm;
      }
      MTP_Printf("device is inserted to list \n");
      return 0;
    }
    
    pt_cur_node = _pt_mtp_attched_device;
    while(NULL != pt_cur_node)
    {
        if(pt_cur_node->params == pt_ptp_parm)
        {
            MTP_Printf("device exist in list already\n");
            return 1;
        }     
        pt_pre_node = pt_cur_node;
        pt_cur_node = pt_cur_node->pt_next;
        
    }  
    pt_cur_node =(mtpdevice_attched_list_t *) malloc(sizeof(mtpdevice_attched_list_t));

    if(pt_cur_node!= NULL)
    {
       memset(pt_cur_node,0,sizeof(mtpdevice_attched_list_t));
       pt_cur_node->params = pt_ptp_parm;
    }
    pt_pre_node->pt_next = pt_cur_node;
...
int  ptp_check_dev_from_list(PTPParams *pt_ptp_parm)
{  
    mtpdevice_attched_list_t *pt_cur_node   = NULL;
    ...
    while(NULL != pt_cur_node)
    {
        if(pt_cur_node->params == pt_ptp_parm)
        {
            MTP_Printf("device has attached\n");
            return 1;
        }     
    ...
    MTP_Printf("device not attached\n");
    return 0;
}

void ptp_release_dev_from_list(PTPParams *pt_ptp_parm)
{  
    mtpdevice_attched_list_t *pt_cur_node   = NULL;
    mtpdevice_attched_list_t *pt_pre_node   = NULL;
    ...
    if(_pt_mtp_attched_device->params == pt_ptp_parm)
    {
      free(_pt_mtp_attched_device);
      _pt_mtp_attched_device = NULL;
      MTP_Printf("device is removed from list\n");
      return;
    }
...
static const LIBMTP_device_entry_t mtp_unsp_device_table[] = {
/* We include an .h file which include some MTP device unsupported  */
#include "unsp-players.h"
};
static const int mtp_unsp_device_table_size = sizeof(mtp_unsp_device_table) / sizeof(LIBMTP_device_entry_t);
						  ...
						  uint32_t bus_location)
{
  ...
  MTP_Printf("append the MTP device\n");
...
#if MTP_DEVICE_CHK_ENHANCE

//internal used structure & binary
typedef struct LIBMTP_device_chk_entry_struct {
 u_int8_t ui1_device_class; /**<device class */
 u_int8_t ui1_interface_class; /**<device interface class */
} LIBMTP_device_chk_entry_t;

static const LIBMTP_device_chk_entry_t mtp_device_chk_table[] = 
{
/* We include an .h file which is shared between us and libgphoto2 */
#include "mtp_device.h"
};
static const int mtp_device_chk_table_size = sizeof(mtp_device_chk_table)/sizeof(LIBMTP_device_chk_entry_t);
static int device_need_check_microsft_description(
                         u_int8_t ui1_device_class,  /**<device class */
                         u_int8_t ui1_interface_class,/**<device interface class */
                         u_int8_t ui1_total_config_num
                         ) 
{
  int i =0;
  //int found = 0;
  MTP_Printf("ui1_device_class    == 0x%x\n",ui1_device_class);
  MTP_Printf("ui1_interface_class == 0x%x\n",ui1_interface_class);
  for(i=0; i<mtp_device_chk_table_size; i++)
  {
  #if !CFG_MASS_STORAGE_FIRST
     if ((0 == ui1_device_class) && 
         (8 == ui1_interface_class) &&
         (1 >= ui1_total_config_num))
     {
       MTP_Printf("ignore 0:8 case for total config as 1\n");
       return 0;
     }
 #endif    
     if((ui1_device_class == mtp_device_chk_table[i].ui1_device_class)&&
        (ui1_interface_class == mtp_device_chk_table[i].ui1_interface_class))
      {
        MTP_Printf("need check microsft descritption\n");
        return 1;
      }
  }
  return 0;
...
static int probe_device_descriptor(struct usb_device *dev, FILE *dumpfile)
{
  ...
  int i_cfg_num =0;
#if MTP_DEVICE_CHK_ENHANCE  
  int int_chk_micro_descrition = 0;
  int i_cur_config =0;
#endif  
  ...
  if(0x05ac == dev->descriptor.idVendor && 0x12 == dev->descriptor.idProduct >> 8)
  {
    return 0;
  } 
   ...
   */
  MTP_Printf("dev config number == 0x%x\n",dev->descriptor.bNumConfigurations);               
  i_cfg_num = dev->descriptor.bNumConfigurations;
#if MTP_DEVICE_CHK_ENHANCE    
  if(0 == usb_get_config(devh,&i_cur_config))
  {
     MTP_Printf("active config == 0x%x\n",i_cur_config);               
  }
  else
  {
     i_cur_config = -1;
     MTP_Printf("Get active config failed \n"); 
  }
#endif
   for (i = 0; i < dev->descriptor.bNumConfigurations; i++)
  {
    ...
    for (j = 0; j < dev->config[i].bNumInterfaces; j++) 
    {
      ...
      {
        MTP_Printf("Check config[%d]:interf[%d]:altset[%d]\n",i,j,k);           
        if (USB_CLASS_PTP == dev->config[i].interface[j].altsetting[k].bInterfaceClass)
        {
            MTP_Printf("interfaceclass == 0x06, so it's a ptp device\n");
#if MTP_DEVICE_CHK_ENHANCE  
            if((i_cfg_num > 1)&&
               (dev->config[i].bConfigurationValue!= i_cur_config)
              )
            {
                MTP_Printf("Curent config value == 0x%x\n\n",dev->config[i].bConfigurationValue);
                MTP_Printf("Erro 2 config not match\n");                
                usb_close(devh);
                return 0;
            }
#endif            
...
#if MTP_DEVICE_CHK_ENHANCE   
        if(!int_chk_micro_descrition)
        { 
           int_chk_micro_descrition =device_need_check_microsft_description(
                  dev->descriptor.bDeviceClass,
                  dev->config[i].interface[j].altsetting[k].bInterfaceClass,
                  dev->descriptor.bNumConfigurations
                 ); 
            MTP_Printf("int_chk_micro_descrition = 0x%x\n",int_chk_micro_descrition);  
         }

 #if CFG_MASS_STORAGE_FIRST
         if(
            ( 0==dev->descriptor.bDeviceClass) &&
            ( 8==dev->config[i].interface[j].altsetting[k].bInterfaceClass)
           )
         {
           MTP_Printf("ignore 0:8 case as MTP\n");
           usb_close(devh);
           return 0;
         }
 #endif
        ...
        ret = usb_get_string_simple(devh, 
				    ...
				    (char *) buf, 
				    1024);
		MTP_Printf("config index = 0x%x\n",i);		    
		MTP_Printf("find usb_get_string_simple return 0x%x\n",ret);		    
	    if (ret < 3)
	    {
            MTP_Printf("ret < 3\n");		    
	        continue;
        ...
        if (strcmp((char *) buf, "MTP") == 0)
        {
            ...
            usb_close(devh);
            MTP_Printf("find MTP device by interace char MTP\n");
            return 1;
        }
...
#if MTP_DEVICE_CHK_ENHANCE
  if((i_cfg_num > 1)&&
     (dev->config->bConfigurationValue!= i_cur_config)
    )
   {
...

  {
      ...
      /* Allocate structs */
       ...
       if (ptp_usb == NULL) 
       {
         MTP_Printf("allocate ptp_usb failed\n");
         usb_close(devh);
         return 0;
       }
       ...
       find_interface_and_endpoints(dev,
    		   &ptp_usb->interface,
    		   &ptp_usb->inep,
       ...
       if(!ptp_usb->inep  ||
          ...
          !ptp_usb->intep)
       {
         int_chk_micro_descrition =0;
         MTP_Printf("not check microsft description for less 3 endpoints\n");
       }
       free(ptp_usb);
  }
  
  if(0 == int_chk_micro_descrition)
  {
      MTP_Printf("Need not check Microsoft device descriptor 0xee:\n");
      usb_close(devh);
      return 0;
  }
#endif  
  /* Read the special descriptor */
  MTP_Printf("Check Microsoft device descriptor 0xee:\n");
  ...
  /* Check if descriptor length is at least 15 bytes */
  if (ret < 15) 
  {
    usb_close(devh);
    return 0;
  ...
  if (!((buf[2] == 'M') && (buf[4] == 'S') &&
	(buf[6] == 'F') && (buf[8] == 'T') &&
	(buf[10] == '1') && (buf[12] == '0') && (buf[14] == '0'))) {
#endif
    usb_close(devh);
    return 0;
  }
      
#if 1 //enable for fixed bugs
  ...
  if (ret <= 0x15) {
    ...
    /* if(ret == -1) {} */
    usb_close(devh);
    MTP_Printf("Check string MTP  failed\n");
    return 0;
  }
...
{
  MTP_Printf("get_mtp_usb_device_list init usb \n");
  struct usb_bus *bus = init_usb();
  MTP_Printf("find bus =0x%x\n", (int)bus);
  for (; bus != NULL; bus = bus->next)
  {
    ...
    for (; dev != NULL; dev = dev->next) 
    {
      if (dev->descriptor.bDeviceClass != USB_CLASS_HUB) 
      {
        ...
        MTP_Printf("get_mtp_usb_device_list find a usb device and not USB HUB\n");
    	...
    	MTP_Printf("device Vendor:Product[0x%x:0x%x]\n",
                    dev->descriptor.idVendor,
                    dev->descriptor.idProduct);
                
        maxloop = (mtp_device_table_size > mtp_unsp_device_table_size) ?
                 mtp_device_table_size: mtp_unsp_device_table_size;  
        for(i = 0; i < maxloop; i++)
        {
          if(i<mtp_device_table_size)
          {
                ...
                dev->descriptor.idProduct == mtp_device_table[i].product_id) 
              {
                ...
                *mtp_device_list = append_to_mtpdevice_list(*mtp_device_list, 
    							...
    							bus->location);
                found = 1;
                MTP_Printf("find the MTP device in table\n");
                break;
              }  
          }

          if(i<mtp_unsp_device_table_size)
          {
              if(dev->descriptor.idVendor == mtp_unsp_device_table[i].vendor_id &&
                dev->descriptor.idProduct == mtp_unsp_device_table[i].product_id) 
              {
                MTP_Printf("un-support device Vendor:Product[0x%x:0x%x]\n",
                    dev->descriptor.idVendor,
                    dev->descriptor.idProduct);
        ...
        {
          int ret =0;
          MTP_Printf("will enter probe_device_descriptor\n");
          ret = probe_device_descriptor(dev, NULL);
          MTP_Printf("probe_device_descriptor return 0x%x\n", ret);
          if (ret) 
          {
  ...
  {
    MTP_Printf("get_mtp_usb_device_list no MTP device \n");
			      ...
			      int * numdevs)
{
  ...
  if (retdevs == NULL) {
    ...
    *devices = NULL;
    *numdevs = 0;
    MTP_Printf("allocated dev number failed\n");
  ...
  while (dev != NULL) 
  {
    ...
    retdevs[i].device_entry.device_flags = 0x00000002U;//0x00000000U;
    // See if we can locate some additional vendor info and device flags
    MTP_Printf("check mtp_device_table_size\n");
...
void dump_usbinfo(PTP_USB *ptp_usb)
{
  ...
  if (devname[0] != '\0') {
    MTP_Printf("Using kernel interface \"%s\"\n", devname);
  }
#endif
  dev = usb_device(ptp_usb->handle);
  MTP_Printf("bcdUSB: %d\n", dev->descriptor.bcdUSB);
  MTP_Printf("bDeviceClass: %d\n", dev->descriptor.bDeviceClass);
  MTP_Printf("bDeviceSubClass: %d\n", dev->descriptor.bDeviceSubClass);
  MTP_Printf("bDeviceProtocol: %d\n", dev->descriptor.bDeviceProtocol);
  MTP_Printf("idVendor: %04x\n", dev->descriptor.idVendor);
  MTP_Printf("idProduct: %04x\n", dev->descriptor.idProduct);
  MTP_Printf("IN endpoint maxpacket: %d bytes\n", ptp_usb->inep_maxpacket);
  MTP_Printf("OUT endpoint maxpacket: %d bytes\n", ptp_usb->outep_maxpacket);
  MTP_Printf("Raw device info:\n");
  MTP_Printf("Bus location: %d\n", ptp_usb->rawdevice.bus_location);
  MTP_Printf("Device number: %d\n", ptp_usb->rawdevice.devnum);
  MTP_Printf("Device entry info:\n");
  MTP_Printf("Vendor: %s\n", ptp_usb->rawdevice.device_entry.vendor);
  MTP_Printf("Vendor id: 0x%04x\n", ptp_usb->rawdevice.device_entry.vendor_id);
  MTP_Printf("Product: %s\n", ptp_usb->rawdevice.device_entry.product);
  MTP_Printf("Vendor id: 0x%04x\n", ptp_usb->rawdevice.device_entry.product_id);
  MTP_Printf("Device flags: 0x%08x\n", ptp_usb->rawdevice.device_entry.device_flags);
...
ptp_read_func (
  ...
  MTP_Printf("Total need read 0x%04lx bytes\n", size);
  ...
  while (curread < size) 
  {
...
#ifdef ENABLE_USB_BULK_DEBUG
    MTP_Printf("Remaining size to read: 0x%04lx bytes\n", size - curread);
#endif
    ...
    {
#ifdef ENABLE_USB_BULK_DEBUG    
      MTP_Printf("unexpected toread size 0x%04x, 0x%04x remaining bytes\n", 
	     (unsigned int) toread, (unsigned int) (size-curread));
#endif
	}
#ifdef ENABLE_USB_BULK_DEBUG
    MTP_Printf("Reading in 0x%04lx bytes\n", toread);
#endif
    result = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep, (char*)bytes, toread, ptp_usb->timeout);
    MTP_Printf("Result of read: 0x%04x\n", result);     
    if (result < 0) 
    {
      return PTP_ERROR_IO;
    }
    ...
    if (ptp_usb->callback_active)
    {
      ...
      {
       	int ret;
       	 MTP_Printf("will enter current_transfer_callback\n");     
   
       	ret = ptp_usb->current_transfer_callback(ptp_usb->current_transfer_complete,
        						 ptp_usb->current_transfer_total,
        						 ptp_usb->current_transfer_callback_data);
         MTP_Printf(" Exit current_transfer_callback\n");     
      ...
      curread % ptp_usb->outep_maxpacket == 0) 
  {
    ...
    MTP_Printf("Zero Read\n");
#endif
    printf("MTP Zero read needed\n");   
    zeroresult = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep, &temp, 0, ptp_usb->timeout);
    if (zeroresult != 0)
      printf("panic: unable to read in zero packet, response 0x%04x", zeroresult);
  }
...
ptp_usb_getdata (PTPParams* params, PTPContainer* ptp, PTPDataHandler *handler)
{
	...
	do {
		...
		if (FLAG_NO_ZERO_READS(ptp_usb) && 
...
#ifdef ENABLE_USB_BULK_DEBUG
		  MTP_Printf("Reading in extra terminating byte\n");
#endif
		  ...
		  if (result != 1)
		    MTP_Printf("Could not read in extra byte for PTP_USB_BULK_HS_MAX_PACKET_LEN_READ long file, return value 0x%04x\n", result);
		} else if (len+PTP_USB_BULK_HDR_LEN == PTP_USB_BULK_HS_MAX_PACKET_LEN_READ && params->split_header_data == 0) {
		  int zeroresult = 0;
		  char zerobyte = 0;

#ifdef ENABLE_USB_BULK_DEBUG
		  MTP_Printf("Reading in zero packet after header\n");
#endif
                  zeroresult = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep, &zerobyte, 0, ptp_usb->timeout);
		  
		  if (zeroresult != 0)
		    MTP_Printf("panic: unable to read in zero packet, response 0x%04x", zeroresult);
		}
...
ptp_usb_getresp (PTPParams* params, PTPContainer* resp)
{
    ...
    MTP_Printf("RESPONSE: ptp_usb_getresp");
	...
	if (ret!=PTP_RC_OK) 
	{   
	    MTP_Printf("RESPONSE: ptp_usb_getpacket failed [0x%x]\n",ret);
		ret = PTP_ERROR_IO;
	}
	else if (dtoh16(usbresp.type)!=PTP_USB_CONTAINER_RESPONSE) 
	{
		MTP_Printf("RESPONSE: PTP_ERROR_RESP_EXPECTED\n");
	...
	if (ret!=PTP_RC_OK) 
	{
        MTP_Printf("RESPONSE: return 0x%x\n",ret);
		return ret;
	}
...
static int init_ptp_usb (PTPParams* params, PTP_USB* ptp_usb, struct usb_device* dev)
{
  ...
  while((device_handle == NULL) && (time_cnt <30))
  {   
      usleep(100);
   MTP_Printf("usb_open()\n");
  device_handle = usb_open(dev);
   MTP_Printf("usb_open out()\n");
      time_cnt++;
  }
  printf("LIBMTP: usb_open try [0x%x]times\n",time_cnt);
  //ptp_usb->timeout = USB_TIMEOUT_DEFAULT;
  {
    if (NULL ==device_handle)
    {
      MTP_Printf("usb_open() failed\n");
      return -1;
    }
    ...
    {
      MTP_Printf("usb_detach_kernel_driver_np()\n");
    ...
    // Only needed on Windows, and cause problems on other platforms.
     MTP_Printf("usb_set_configuration()\n");
    if (usb_set_configuration(device_handle, dev->config->bConfigurationValue)) 
    {
      MTP_Printf("usb_set_configuration() failed\n");
      return -1;
    }
#endif
    MTP_Printf("usb_claim_interface()\n");
    if (usb_claim_interface(device_handle, (int) ptp_usb->interface))
    {
      MTP_Printf("usb_claim_interface()failed\n");
      return -1;
    }    
  }
  return 0;
...
static void clear_stall(PTP_USB* ptp_usb)
{
  ...
  if (ret<0) {
    ...
    MTP_Printf("Clearing stall on IN endpoint\n");
  ...
  if (ret<0) {
    ...
    MTP_Printf("Clearing stall on OUT endpoint\n");
					 ...
					 int* intep)
{
  ...
  MTP_Printf("bNumConfigurations = 0x%x\n",dev->descriptor.bNumConfigurations);
  for (i = 0; i < dev->descriptor.bNumConfigurations; i++) 
  {
    uint8_t j;
    MTP_Printf("config[0x%x]-inteface number:0x%x configvalue:0x%x\n",
               i,
    ...
    for (j = 0; j < dev->config[i].bNumInterfaces; j++) 
    {
      ...
      *interface = dev->config[i].interface[j].altsetting->bInterfaceNumber;
      ...
      MTP_Printf("endpoint %d:%d:%d\n",*inep,*outep,*intep);
      MTP_Printf("pointMax %d:%d\n",inep_maxpacket,outep_maxpacket);  
      return;
    }
  }
}
					   ...
					   void **usbinfo)
{
  ...
  for (; bus != NULL; bus = bus->next)
  {
    if (bus->location == device->bus_location) 
    {
      ...
      MTP_Printf("Check device vendor\n");
      for (; dev != NULL; dev = dev->next) 
      { 
         
        MTP_Printf("Got: %d-%d-%d\n",dev->devnum,dev->descriptor.idVendor,dev->descriptor.idProduct);
    	MTP_Printf("In : %d-%d-%d\n",device->devnum,device->device_entry.vendor_id,device->device_entry.product_id);
  ...
  {
    MTP_Printf("init_ptp_usb failed\n");
  ...
  {
    MTP_Printf("PTP_RC_InvalidTransactionID failed\n");
  ...
  {
    MTP_Printf("PTP_RC_InvalidTransactionID  failed3\n");
