...
{   
	...
	/* send request */
	MTP_Printf("will Enter sendreq_func \n");
	ui2_ret = params->sendreq_func (params, ptp);
	MTP_Printf("out sendreq_func \n");
	if(PTP_RC_OK !=ui2_ret)
	{
       return ui2_ret;
    }
	...
	case PTP_DP_GETDATA:
		{
			uint16_t ret;
			MTP_Printf("will Enter getdata_func \n");
			ret = params->getdata_func(params, ptp, handler);
			MTP_Printf("getdata_func return 0x%x\n",ret);
			if (ret == PTP_ERROR_CANCEL)
			{
				ret = params->cancelreq_func(params, 
							     params->transaction_id-1);
	...
	case PTP_DP_NODATA:
		break;
	default:
	    //ptp_release_transaction_sem(params->pSem);  				 
		return PTP_ERROR_BADPARAM;
	}
	/* get response */
	MTP_Printf("will Enter getresp_func \n");
	ui2_ret = params->getresp_func(params, ptp);
	MTP_Printf("out Enter getresp_func \n");
	if(PTP_RC_OK !=ui2_ret)
	{
       return ui2_ret;
    }
...
ptp_transaction (PTPParams* params, PTPContainer* ptp, 
	...
	uint16_t	ret;
    MTP_Printf("Enter %s \n",__FUNCTION__);
    ret = ptp_wait_transaction_sem(params->pSem);  
	if(PTP_RC_OK != ret)
	{
	    return PTP_RC_InvalidSemHandle;
	}	
	switch (flags & PTP_DP_DATA_MASK) {
	case PTP_DP_SENDDATA:
	...
	MTP_Printf("will Enter ptp_transaction_new \n");
	ret = ptp_transaction_new (params, ptp, flags, sendlen, &handler);
    MTP_Printf(" ptp_transaction_new return 0x%x\n",ret);
	switch (flags & PTP_DP_DATA_MASK) {
	case PTP_DP_SENDDATA:
...
ptp_closesession (PTPParams* params)
{
	PTPContainer ptp;
    MTP_Printf("Enter %s \n",__FUNCTION__);
    uint16_t ret = ptp_wait_transaction_sem(params->pSem);  
	if(PTP_RC_OK != ret)
	{
	    return PTP_RC_InvalidSemHandle;
	}	
...
ptp_getpartialobject (PTPParams* params, uint32_t handle, uint32_t offset,
			uint32_t maxbytes, unsigned char** object, uint32_t *readbytes)
{
	PTPContainer ptp;
	...
	ptp.Nparam=3;
	*readbytes=0;
	return ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, object, readbytes);
}
...
ptp_ek_sendfileobject_from_handler (PTPParams* params, PTPDataHandler*handler, uint32_t size)
{
	PTPContainer ptp;
    uint16_t ui2_ret =0;
    ui2_ret = ptp_wait_transaction_sem(params->pSem);  
	if(PTP_RC_OK != ui2_ret)
	{
	    return PTP_RC_InvalidSemHandle;
	}	
	...
	ui2_ret = ptp_transaction_new(params, &ptp, PTP_DP_SENDDATA, size, handler);
    ptp_release_transaction_sem(params->pSem);  
	...
	PTPDevicePropDesc *dpd)
{
	...
	if (dpd->FormFlag == PTP_DPFF_Enumeration) {
        ...
        if (dpd->FORM.Enum.SupportedValue == NULL)
    	{
    	    MTP_Printf("%s Line %d allocated memory failed\n" ,__FUNCTION__,__LINE__);
    	    return PTP_RC_Undefined;
        }
...
uint16_t 
ptp_create_transaction_sem(char *sem_name,sem_t **ppSem)
{
  int i_curvalue =0;
  MTP_Printf("create Sem [%s]\n",sem_name);
  *ppSem = sem_open(sem_name,O_CREAT,S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH,1); 

  if(*ppSem == SEM_FAILED || *ppSem == NULL) 
  { 
     *ppSem = NULL;
     MTP_Printf("create Sem falied\n");  
     return PTP_RC_InvalidSemHandle;
  } 
  sem_getvalue(*ppSem,&i_curvalue);
  MTP_Printf("current Sem is [%d]\n",i_curvalue);  
  while(i_curvalue == 0)
  {
    //MTP_Printf("create Sem falied\n");  
    sem_post(*ppSem);
    sem_getvalue(*ppSem,&i_curvalue);
    MTP_Printf("re-set Sem is [%d]\n",i_curvalue);  
  }
...
uint16_t 
ptp_del_transaction_sem(sem_t *pSem)
{
  int i_curvalue =0;
  MTP_Printf("delete Sem\n");

  if(NULL == pSem)
  {
     MTP_Printf("Erro invalid sem handle \n");  
     return PTP_RC_InvalidSemHandle;
  }
    
  if(pSem)
  {
    sem_getvalue(pSem,&i_curvalue);
    MTP_Printf("current Sem is [%d]\n",i_curvalue);  
    sem_wait(pSem);
    sem_destroy(pSem);
    pSem = NULL;
  }
  MTP_Printf("Sem deleted\n");  
  return PTP_RC_OK;
}

uint16_t 
ptp_wait_transaction_sem(sem_t *pSem)
{
    int ret =0; 
    //MTP_Printf("LIBMTP: start got Sem \n");
    if(NULL == pSem)
    {
       MTP_Printf("Erro invalid sem handle \n");  
       return PTP_RC_InvalidSemHandle;     
    ...
    {
    #if 0//(_POSIX_TIMEOUTS != -1)
        #include "time.h"
        uint32_t mswait = 2000;   
        timespec ts = {0};  
        MTP_Printf("sem timed wait Semphore\n"); 
        ...
        if(ret!=0)
        {
          MTP_Printf("Got sem failed \n");
          return PTP_RC_Undefined;
        }
        MTP_Printf("have got sem \n");
    #else  
         MTP_Printf("sem wait Semphore\n"); 
         ret = sem_wait(pSem);
         
         if(ret!=0)
         { 
           MTP_Printf("LIBMTP: Got sem failed \n");
           return PTP_RC_Undefined;
         }
         MTP_Printf("have got sem \n");
    #endif  
...
uint16_t 
ptp_release_transaction_sem(sem_t *pSem)
{
    if(pSem)
    {
       MTP_Printf("Release Semphore\n");  
       sem_post(pSem); 
       MTP_Printf("Released Semphore\n");       
    }
    return PTP_RC_OK;
}
