...
#-------------------------------------------------------------------------------
	...
	unset DEST_DIR
	export DEST_DIR=$(pwd)/../../../library/qt/qt-everywhere-opensource-src-4.6.2

#-------------------------------------------------------------------------------
# Check whether need to extract symbolic_lib.tar.bz2
#-------------------------------------------------------------------------------
	if [ ! -e ${DEST_DIR}/lib/libQtCore.so ]; then
		if [ -e ${DEST_DIR}/lib/symbolic_lib.tar.bz2 ]; then
			tar xjf ${DEST_DIR}/lib/symbolic_lib.tar.bz2 -C ${DEST_DIR}/lib
			MAKE_RET=$?
			if [ ${MAKE_RET} -ne 0 ]; then
				echo
				echo -e "\033[40;31mExtract the File ${DEST_DIR}/lib/symbolic_lib.tar.bz2 Fail...(${MAKE_RET})\033[0m"
				echo
				exit ${MAKE_RET}
			fi
		else
			echo
			echo -e "\033[40;31mThe File ${DEST_DIR}/lib/symbolic_lib.tar.bz2 Not Found...\033[0m"
			echo -e "\033[40;31mPlease Get It From P4v\033[0m"
			echo
			exit 12
		fi
	fi
...
#-------------------------------------------------------------------------------
	...
	export DFB_DIR=$(pwd)/../../../library/DirectFB/DirectFB-1.2.7
