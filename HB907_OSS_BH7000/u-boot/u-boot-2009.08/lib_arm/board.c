...
#		endif
	...
	/* mtk partition table init */
	mt85xx_part_tbl_init();

	/* check upg status */
	/* printf on/off */
	if(i_get_upg_status(&r_upg_status) != UPG_STATUS_OK)
	{
	    puts("[MT85XX Boot] Error: can't find upg status\n");
           disable_print=0;
	}
	else if ((r_upg_status.u4_log_cfg & 0xFF) != 0)
	{
		
            BIM_WRITE32(REG_RW_GPRB2, r_upg_status.u4_log_cfg);
            disable_print=r_upg_status.u4_log_cfg;
	 }
#endif

#if ACON_STANDBY
	puts("[MT85XX Boot] Standby on starts\n");
	/* Checking standby mode */
	v_mt85xx_standby();
	puts("[MT85XX Boot] Standby on ends\n");
#endif
...
#endif
    //mtk40143
    REG_SER_INT_EN = REG_SER_INT_EN | 0x1;
