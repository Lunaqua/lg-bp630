...
//#define MT539X_LAN8700
 ...
 #define MT85XX_STAR_MAC
#endif


#ifdef MT539X_LAN8700
#define DEFAULT_PHY_ADDRESS 22
#elif defined(MT85XX_STAR_MAC)
...
#define MAC_BASE                        (ETHERNET_BASE + MAC_REG_OFFSET)  
#define MMC_BASE                        (ETHERNET_BASE + MMC_REG_OFFSET) 
#define DMA_BASE                        (ETHERNET_BASE + DMA_REG_OFFSET) 
...
#define PINMUX_BASE                         (IO_BASE + 0x24000)
#define IPLL1_BASE                          (IO_BASE + 0x5c700)
...
enum MacInitReg
{
  ...
  DMA_OPMODE_INIT      	= (UINT32)(RX_OFFSET_2B_DIS | RX_INTR_WDLE | TX_INTR_WDLE | TX_SUSPEND | RX_SUSPEND),       // Operation Mode  
...
#if (!CONFIG_DRV_MT8520)
#define vWriteMACPDWNC(dAddr, dVal)  IO_WRITE((ETHERNET_PDWNC_REG_OFFSET+dAddr), (dVal))
#define dReadMACPDWNC(dAddr)         IO_READ((ETHERNET_PDWNC_REG_OFFSET+dAddr))
...
//  Mmac mmac;            /* mmac device internal data */
//  DmaDesc  desc[512];         /* DMA descriptors - 128 rx and 128 tx */
  DmaDesc  desc[NUM_TX_DESC+NUM_RX_DESC];         /* DMA descriptors - 2 rx and 2 tx */
  Mmac mmac;            /* mmac device internal data */
  //struct mii_if_info mii;
  //struct timer_list	timer;
} Private;
