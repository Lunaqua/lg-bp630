/* Mtk internal MAC uboot driver
 *
 *
 */
#include <chip_ver.h>
#include <asm/arch/mt85xx_mac.h>
//#include "../board/mt5391/drv_lib/drv_inc/c_model.h"
//#include "c_model.h"


#define DEFAULT_MAC_ADDRESS { 0x00, 0x0c, 0xe7, 0x00, 0x00, 0x00 }
...
{
    INT32 i4Ret,u4Data,u1InterPhyAddr;

				   /* Init IPLL for internal PHY if necessary */
#if(!USE_EXT_CLOCK_SOURCE)  
				#if (CONFIG_CHIP_VER_CURR != CONFIG_CHIP_VER_MT8550)
				
					/* perform the following settings only when Internal PHY is used */
					u32 data = IO_READ(PINMUX_BASE + 0x34);
					if ((data & 0x01) == 0)
					{
						IO_WRITE((PINMUX_BASE+0x30), 0x85); /* FERST_CODE */
						IO_WRITE(PINMUX_BASE+0x34, 0x1); /* FE_RST */
						
						IO_WRITE(IPLL1_BASE+0x04, 0x801);
						data = IO_READ(IPLL2_BASE+0x10);
						data = (data&0xffffff00)|((2<<0)|(2 <<4));
						IO_WRITE(IPLL2_BASE+0x10, data);
					}
				#endif
#endif
		...
		/* Token ring */	
		//work around for long cable link fail issue. It observed on 8555, 55 will ECO for it, but it never observed on 8550.
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8550) 
		i4MacPhyWrite(u4PhyAddr, 0x1f, 0x52b5);    
		i4MacPhyWrite(u4PhyAddr, 0x12, 0x0);	
		i4MacPhyWrite(u4PhyAddr, 0x11, 0x2000);    
		i4MacPhyWrite(u4PhyAddr, 0x10, 0x8258);    
#endif    
		//test page
		i4MacPhyWrite(u4PhyAddr, 0x1F, 0x2a30); 
		i4MacPhyWrite(u4PhyAddr, 0x03, 0x000c); //10M power saving
		i4MacPhyWrite(u4PhyAddr, 0x08, 0x0210); //disable gating of RCLK125 according to YT's suggest
		i4MacPhyWrite(u4PhyAddr, 0x0E, 0x35); //analog
		i4MacPhyWrite(u4PhyAddr, 0x18, 0xE416);  //analog
    ...
    #if 1//def INT_PHY_LED_POL_INV
		i4MacPhyRead(u4PhyAddr, 0x10, &u4Data); //LED_POL default inverse
		u4Data |= (1<<2);
		i4MacPhyWrite(u4PhyAddr, 0x10, u4Data);
    #endif    
    ...
    if(i4Ret != NET_DRV_OK)
    {
        TR("ethernet internal PHY init fail\n");
    }
...
void vMacInit(void)
{
	u32 u4Data;//bit3~bit7
	u8  u8PhyAddr=0;

   #if (!INTERNAL_PHY)
	u8PhyAddr = u8MacPhyAddrGet();
   #endif
				...
				vWriteChkSum(RW_INTERNAL_PHY_CTRL,u4Data|(u8PhyAddr <<INTERNAL_PHY_ADDRESS_POS));
			#if (INTERNAL_PHY)
			...
			#else
                ClrBitChkSum(RW_INTERNAL_PHY_CTRL, INTERNAL_PHY_ENABLE_MDC_MDIO);
			    ClrBitChkSum(RW_INTERNAL_PHY_CTRL, INTERNAL_PHY_ENABLE_MII_PIN);
				ClrBitChkSum(RW_INTERNAL_PHY_CTRL1,SOFT_RESET_DISABLE);
			#endif
				
				
   #else
				u4Data= (dReadMAC(RW_INTERNAL_PHY_CTRL)& (~INTERNAL_PHY_ADDRESS_MASK));
				vWriteMAC(RW_INTERNAL_PHY_CTRL,u4Data|(u8PhyAddr <<INTERNAL_PHY_ADDRESS_POS));
			#if (INTERNAL_PHY)
			...
			#else
                ClrBitMAC(RW_INTERNAL_PHY_CTRL, INTERNAL_PHY_ENABLE_MDC_MDIO);
                ClrBitMAC(RW_INTERNAL_PHY_CTRL, INTERNAL_PHY_ENABLE_MII_PIN);
				ClrBitMAC(RW_INTERNAL_PHY_CTRL1,SOFT_RESET_DISABLE);
			#endif	
    ...
    vWriteMAC(RW_EXT_CFG, MAC_EXT_INIT); //Little endian 2010/3/2
    #endif
    /* Init MAC setting */
...
{
//	u32 base = dev->base;
    vWriteMAC(RW_C_RXOKPKT, 0);
    vWriteMAC(RW_C_RXOKBYTE, 0);
    vWriteMAC(RW_C_RXRUNT, 0);
    vWriteMAC(RW_C_RXLONG, 0);
    vWriteMAC(RW_C_RXDROP, 0);
    vWriteMAC(RW_C_RXCRC, 0);
    vWriteMAC(RW_C_RXARLDROP, 0);
    vWriteMAC(RW_C_RXVLANDROP, 0);
    vWriteMAC(RW_C_RXCSERR, 0);
    vWriteMAC(RW_C_RXPAUSE, 0);
    vWriteMAC(RW_C_TXOKPKT, 0);
    vWriteMAC(RW_C_TXOKBYTE, 0);
    vWriteMAC(RW_C_TXPAUSE, 0);
}

#if (!INTERNAL_PHY)
void vPinMuxInit(void)
{
  IO_WRITE((PINMUX_BASE+0xF8), IO_READ((PINMUX_BASE+0xF8))|0x007ffe60); ///enable ethernet external PHY IO pin
  IO_WRITE((PINMUX_BASE+0xF8), IO_READ((PINMUX_BASE+0xF8))|0x007ffe60); ///enable ethernet external PHY IO pin
}
#endif
...
void hwcfgsetup(void)
{
  ...
  */
  ...
  PDWNC_WRITE32(PDWNC_REG_UP_CONF_OFFSET, PDWNC_READ32(PDWNC_REG_UP_CONF_OFFSET)&(~UP_CONF_ETEN_EN));
...
//  vWriteMAC(RW_PHY_CTRL1, PHY_CTRL_INIT | ((0 << PHY_ADDR_AUTO_SHIFT) & PHY_ADDR_AUTO));  
  ...
  vWriteMACPDWNC(RW_EXT_CFG,PDWNC_MAC_EXT_INIT);
  vWriteMAC(RW_EXT_CFG, MAC_EXT_INIT & (~(HRST|MRST|NRST)));
  vWriteMAC(RW_EXT_CFG, MAC_EXT_INIT );
...
int StarDmaTxSet(Mmac *dev, u32 buffer, u32 length, u32 extBuf)
{
	...
	txDesc->ctrlLen &= ~TX_COWN;     /* Set HW own */
	 ...
	 TR(  "Mmac::send () decIdx=%d; buffer=%x; len=%x; \n",descIdx,buffer,length );
...
int StarDmaRxGet(Mmac *dev /*, u32 *buffer, u32 *ctrlLen, u32 *extBuf */)
{
	...
	if (StarDmaRxValid(rxDesc->ctrlLen)) /* process good packets only */
    {
        ...
        TR(  "receive good packet,addr=%x ; len=%d \n",rxDesc->buffer,len);
		NetReceive(rxDesc->buffer, len);
    }
...
//    dev->name[0] = 'M';
	...
	/*
	{u32 u4DataTmp;
	  if(i4MacPhyRead(0, MII_PHY_ID1_REG, &u4DataTmp))
	      TR(  "Mmac::0ID =%x \n",u4DataTmp);
	  if(i4MacPhyRead(1, MII_PHY_ID1_REG, &u4DataTmp))
	     TR(  "Mmac::1ID =%x \n",u4DataTmp);
    }
	*/
...
void check_media(struct eth_device *dev, Private *pr, Mmac *mmac)
{
  ...
  int duplex;
  int i;
  UINT32 u4CtrlReg0,u4CtrlReg1; 
  UINT32 u4AnlparReg5,u4AnlparReg4;
...
/**********************************************************
 * Register address offset definition
 **********************************************************/
#define ETHERNET_PDWNC_BASE        (CKGEN_BASE + 0x24C00)
//#define MAC_PDWNC_REG_OFFSET	     (ETHERNET_PDWNC_BASE + 0x000)
#define RW_MAC_PAD_SEL		0xC0					/* Select pad for pdwnc or main ethernet */
/* Only can be setted in PDWNC ethernet register */  
#define PAD_FOR_PDWNC 	((u32)0x01 << 31)   	/* Pad for PDWNC ethernet */
#define CLK_FROM_27M      ((u32)0x01 << 30)   	/* TX_CLK & RX_CLK from 27M */
#define RW_MAC_DRAM_PRIORITY	0xC0				/* DRAM low/high priority */
/* Only can be setted in Main ethernet register */  
#define DRAM_LOW_PRIORITY	((u32)0x01 << 30)   	/* DRAM low priority */ 
...
//    u32 data;
    ...
    //data = __raw_readl(PINMUX0_BASE);
    //data |= (0x03 << 30);
    //__raw_writel(data, PINMUX0_BASE);           //0x7000D0C0[31:30] = b'11

    u32 data1;

/* switch PDWN ethernet register access register to ARM */     	
  data1 = __raw_readl(ETHERNET_PDWNC_BASE + RW_MAC_PAD_SEL);

  data1 &= ~(PAD_FOR_PDWNC | CLK_FROM_27M);
  
  __raw_writel(data1, ETHERNET_PDWNC_BASE + RW_MAC_PAD_SEL);  
...
//	udelay(500);
      ...
      {
        if(MmacMiiRead(Dev, PHY_BMCR) != 0xFFFF)
        {
          TR("Mmac::PHY ID = %d\n", Dev->phyAddr );
          break;
        }
