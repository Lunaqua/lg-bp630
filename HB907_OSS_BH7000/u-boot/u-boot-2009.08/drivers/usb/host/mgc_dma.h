...
#if 0
#define DMA_BASE_ADDRESS(core) \
        MGC_DRC_Core_Objects[core].BaseAddress + 0x200

#define DMA_INTR_REGISTER(core) USB_DMABASE

#define DMA_CNTL_REGISTER(core, channel) \
        (__u32 *)(USB_DMABASE + 0x10 * (channel - 1) + 0x4)

#define DMA_ADDR_REGISTER() \
        (__u32 *)(USB_DMABASE + 0x10 * (channel - 1) + 0x8)

#define DMA_COUNT_REGISTER(core, channel) \
        (__u32 *)(USB_DMABASE + 0x10 * (channel - 1) + 0xC)
#endif
