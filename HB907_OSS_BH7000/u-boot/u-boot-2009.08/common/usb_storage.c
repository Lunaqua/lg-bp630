...
#ifndef USB_MAX_STOR_DEV
#define USB_MAX_STOR_DEV 6
#endif
static int usb_max_devs = 0; /* number of highest available usb device */
static int usb_max_lun = 0;
...
static int usb_stor_get_max_lun(struct us_data *us, int *lun)
{
	int result;
	...
	result = usb_control_msg(us->pusb_dev, usb_rcvctrlpipe(us->pusb_dev,0),
				0xFE, 0xA1,
				0, 0, (void *)lun, 1, USB_CNTL_TIMEOUT*5);
        if (result < 0)
        {
            *lun = 0;
        }
...
again:         
	...
	if(mode==1) {
		printf("\n(%d) scanning bus for storage devices...", retry);
	}
...
//  MTK Notice: Max Liao, 2007/12/17.
//  CR-0007: HIYATEK card reader.
//  CR-0033: MotionFlash card reader need a delay before query device.	
//  CR-0040: Sandisk card reader need a delay before query device.	
                wait_ms(1000);
                
	        ss = &usb_stor[usb_max_devs];
		if(usb_storage_probe(dev, 0, ss)) { /* ok, it is a storage devices */

                        usb_max_lun = 0;
        		if (1 != usb_stor_get_max_lun((struct us_data *)dev->privptr, &usb_max_lun))
        		{
                            USB_STOR_PRINTF("Get Lun Fail.\n");
                            break;                
        		}
        		usb_max_lun ++;
                        USB_STOR_PRINTF("Max Lun = %d.\n", usb_max_lun);

			/* get info and fill it in */
			for (j=0; j<usb_max_lun; j++)
			{
                            usb_dev_desc[usb_max_devs].lun = j;
                            result = usb_stor_get_info(dev, ss, &usb_dev_desc[usb_max_devs]);
                            if(result > 0)
                            {
				usb_max_devs++;
                            }
...
int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
   ...
   again:
	...
	if((result < 0) && (us->pusb_dev->status == USB_ST_NAK_REC)) {
		printf("STATUS:nak = %d\n", retry);
		if (retry++ < 5)
		{
		    /* do a retry */
...
static int usb_test_unit_ready(ccb *srb,struct us_data *ss)
{
	...
	do {
		...
		usb_request_sense (srb, ss);
                    ...
                    // maybe medium not ready, wait it.
                    if ((srb->sense_buf[2] == MGC_SCSI_SC_UNIT_ATTENTION) &&
                         (srb->sense_buf[12] == MGC_SCSI_ASC_MEDIUM_CHANGE))
                    {
                        if (retries > 0)
		wait_ms (100);
                    }
...
//  disable set_interface in mass storage device.	
...
//  disable bInterfaceSubClass check.	
#if 0	 
