...
//mtk40224- follow code for match a file in the dir
int do_matchfile_from_dir (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int ret = 0;
    char filename[256] ={0};

    //printf("command matchfile argc=%d , argv[1]=%s, argv[2]=%s, argv[3]=%s, argv[4]=%s.\n", argc, argv[1], argv[2], argv[3], argv[4]);
	if (argc < 5)
    {
		printf ("usage: filesearch <interface> <dev[:part]> [directory] [match file name]  .\n");
		return (0);
	}

    printf("call x_match_file_from_dir .\n");
    ret = x_match_file_from_dir(argv[1], argv[2],argv[3], argv[4], filename);

    printf("find file name is %s.\n", filename);
	if(ret!=0)
	{
		printf("call x_match_file_form_dir return fail ret=%d. \n", ret);
	}
    else
    {
        printf("call x_match_file_form_dir return success. \n");
    }
...
U_BOOT_CMD(
	matchfile,	5,	1,	do_matchfile_from_dir,
	"match a file  in a directory",
	"<interface> <dev[:part]> [directory] [match file name]\n"
	"    - match a file from 'dev' on 'interface' in a 'directory'"
);
