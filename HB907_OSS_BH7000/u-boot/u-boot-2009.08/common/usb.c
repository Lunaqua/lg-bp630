...
int usb_se0nak(void)
{
	...
	/* if lowlevel init is OK, set SE0 NAK test */
	if(result==0) {
		usb_sendse0nak();
		return 0;
	}
...
int usb_electrical_test(char *argv)
{
	int result;

        printf("Enter USB Electrical Test mode...\n");
        
	/* init low_level USB */
	result = usb_lowlevel_init();	
	/* if lowlevel init is OK, send test packet */
	if(result==0) {
            if (strncmp(argv, "se0nak", 6) == 0){
                return usb_sendse0nak();
            }

            if (strncmp(argv, "testj", 5) == 0){
                return usb_sendtestj();
            }

            if (strncmp(argv, "testk", 5) == 0){
                return usb_sendtestk();
            }

            if (strncmp(argv, "testpkt", 7) == 0){
                return usb_sendtestpacket();
            }
	}
...
int usb_silent_init(void)
{
	...
	usb_hub_reset();
	/* init low_level USB */
	printf("USB Silent Init:   ");
	result = usb_silent_reset();
...
int usb_hub_configure(struct usb_device *dev)
{
	...
	struct usb_port_status portsts;
	unsigned short portstatus = 0, portchange = 0, bIsFoundDeviceOnPort = 0;
	...
	USB_HUB_PRINTF("wait to power good time: %dms\n",
			descriptor->bPwrOn2PwrGood * 2);
	...
	for (j = 0; j < 10; j++)
	{	
		USB_HUB_PRINTF("Get Port Status (%d)\n", j+1);
		...
		if(bIsFoundDeviceOnPort ) 
	    {
			USB_HUB_PRINTF("Found device(s)!\n");
	    		break;
	    }
	    else
	    	{
			USB_HUB_PRINTF("Device not found !\n");   		
	    	}
	}/* end for j all ports */
